...
def score_calculate(ans_list, predict_list):
    ...
    return temp_em, temp_inter, temp_precision, temp_recall, temp_f1
...
def main(ans_file = 'ans.json', predict_file = 'predict.json'):
    ...
    # 總分 list
    em_total, inter_total, prec_total, rec_total, f1_total = ([] for i in range(5))
        ...
        for pred_defendant in predict:
            if ans_defendant['content_id'] == pred_defendant['content_id']:
                ...
                if ans_defendant['name'] == pred_defendant['name']:
                    ...
                    em_loc_temp, inter_loc_temp, prec_loc_temp, rec_loc_temp, f1_loc_temp = score_calculate(ans_defendant['job_location'], pred_defendant['job_location'])
                    em_tit_temp, inter_tit_temp, prec_tit_temp, rec_tit_temp, f1_tit_temp = score_calculate(ans_defendant['job_title'], pred_defendant['job_title'])
                    em_law_temp, inter_law_temp, prec_law_temp, rec_law_temp, f1_law_temp = score_calculate(ans_defendant['laws'], pred_defendant['laws'])
